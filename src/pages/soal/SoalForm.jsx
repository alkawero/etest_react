...
import { EditorState, convertFromHTML, blocksFromHTML, ContentState, createFromBlockArray } from 'draft-js';
...
const SoalForm = ({ create, update, onClose, soal, action, open }) => {
    ...
    const addAnswers = (answer) => {
        let exist = answers.filter(a => (a.code === answer.code || a.content === answer.content)).length
        if (exist < 1) {
    ...
    const [questionEditor, setQuestionEditor] = useState(EditorState.createEmpty())
    ...
    const getDataKd = async () => {
        ...
        if (response.data) {
            setDataKd(response.data.map(data => ({ id: data.id, value: data.kd, description: data.kd_desc, meta: `kd : ${data.kd} | semester : ${data.semester}` })));
        }
    }
    ...
    const getDataRanah = async (kds) => {
        ...
        setDataRanah(response.data.map(data => ({ id: data.ranah_id, value: data.ranah_code, description: `${data.ranah_code} - ${data.ranah_ket}`, meta: `kd : ${data.kd}` })));
    }
    ...
    const getDataIndicator = async (ranahs) => {
        ...
        setDataIndicator(response.data.map(data => ({ id: data.trx_indi_id, value: data.indikator, description: data.indikator, meta: `ranah : ${data.ranah}` })));
    ...
    const getDataLevel = async () => {
        ...
        setDataLevel(response.data.map(j => ({ label: j.value, value: j.num_code })));
    }
    ...
    const getDataContentTypes = async () => {
        ...
        const response = await doGet('param', params)
        setDataContentTypes(response.data.map(data => ({ id: data.num_code, value: data.value, code: data.num_code })));
    ...
    useEffect(() => {
        if (soal) {
            ...
            setJenjang({ label: soal.jenjang, value: soal.jenjang, load: 'first-load' })
            setGrade({ label: soal.grade_num, value: soal.grade_char, load: 'first-load' })
            setSubject({ label: soal.subject_name, value: soal.subject_id, load: 'first-load' })
            setKd(
                soal.kds.map(kd => ({ id: kd.id, value: kd.kd, description: kd.kd_desc, load: 'first-load' }))
            )
            setRanah(
                soal.ranahs.map(ranah => ({ id: ranah.id, value: ranah.ranah_kode, description: `${ranah.ranah_kode} - ${ranah.ranah_ket}`, meta: `jenis : ${ranah.ranah_jenis}`, load: 'first-load' }))
            )
            setIndicator(
                soal.indicators.map(data => ({ id: data.id, value: data.indi_pencapaian, description: data.indi_pencapaian, meta: `ranah : ${data.indi_ranah}`, load: 'first-load' }))
            )
            ...
            if (soal.answer_type === 'M') {
                setAnswers(soal.options.map(op=>({code:op.code,
                                                    content: op.content,
        ...
        <>
            <Grid container direction='column' className={classes.addContent}>
                ...
                <Grid container className={common.paddingX}>
                    ...
                    <span style={{ color: 'red', margin: '0 16px' }}>{errorState.ekd}</span>
                    <span style={{ color: 'red', margin: '0 16px' }}>{errorState.eranah}</span>
                    <span style={{ color: 'red', margin: '0 16px' }}>{errorState.eindicator}</span>
                    <span style={{ color: 'red', margin: '0 16px' }}>{errorState.elevel}</span>
                ...
                <Grid container className={clsx(classes.editorWrapper, common.marginBottom)}>
                    <Grid container justify='space-evenly' wrap='nowrap' className={classes.editorHeader}>
                ...
                <Grid container className={classes.editorWrapper}>
                    <Grid container justify='space-evenly' wrap='nowrap' className={classes.editorHeader}>
                        <Grid container justify='flex-start' alignItems='center'>
                            <Chip size="small" label="Question" className={common.marginTop} />
                        </Grid>
                    ...
                    <Grid container className={classes.inputWraper}>
                        ...
                        <Conditional condition={questionType === 3}>
                            <MathInput value={mathQuestion} type='asciimath' action={setMathQuestion} />
                ...
                <Grid container className={classes.answerTypeWrapper}>
                    <FormControl component="fieldset" className={classes.formControl}>
                        ...
                        >
                            <FormControlLabel value="E" control={<Radio color="primary" />} label="Essay" />
                            <FormControlLabel value="M" control={<Radio color="primary" />} label="Multiple Choice" />
                        </RadioGroup>
                    </FormControl>
                </Grid>
                    ...
                    <Grid container className={classes.editorWrapper}>
                        <Grid container justify='space-evenly' wrap='nowrap' className={classes.editorHeader}>
                            ...
                            <Grid item xs={8} container alignItems='center'>
                                <Conditional condition={answerType === 'M' && rightAnswer === '' && answers.length > 0}>
                        ...
                        <Grid container direction='column' style={{ padding: 8 }}>
                            {answers.map(answer => (
                                <Grid key={answer.code} container spacing={1} className={classes.hoverable}>
                                    <Grid container alignItems='center' item xs={1}><Avatar className={classes.avatar}>{answer.code}</Avatar></Grid>
                                    ...
                                    <Grid item xs={1} container alignItems='center'>
                                        <DeleteButton tooltip='remove answer' action={() => deleteAnswer(answer)} />
                                    </Grid>
                                </Grid>
                            ))}
                        </Grid>
            ...
            <PopUp anchor={answerFormAnchor} position={'top-end'}>
                <AnswerForm save={addAnswers} cancel={() => { setAnswerFormAnchor(null) }} />
            </PopUp>
        </>
