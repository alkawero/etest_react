...
import { doUpload, doPost, doGet } from 'apis/api-service';
...
const SoalForm = ({page,create,update,onClose}) => {
    ...
    const contenTypes = [{id:1,value:'plain text',code:1}, {id:2,value:'rich text',code:2},{id:3,value:'equation',code:3}]
    ...
    const getDataIndicator =async()=>{
        const kd_ids = kd.map(kd=>(kd.id))
        const ranah_codes = ranah.map(ranah=>(ranah.value))
        
        const params= {selection:1,
        kd_ids:kd_ids,
    ...
    useUpdateEffect(() => {
        if(answers.length>0){
            errors = {...errors,echoices:''}
            setErrorState({...errorState,echoices:''}) 
        }
        if(rightAnswer!==''){
            errors = {...errors,erightAns:''}
    ...
    useUpdateEffect(() => {
        setKd([])
        setDataKd([])
        if(jenjang!==null && grade!==null && subject!==null){
            getDataKd()
        }        
    },[subject])
        ...
        <>
            <Grid container direction='column' className={classes.addContent}>
                ...
                <Grid container className={common.paddingX}>
                    ...
                    <MultipleSelect
                        options={dataKd}
                        setSelectedData={setKd}
                        placeholder="kd"
                    />
                    
                    <MultipleSelect
                        options={dataRanah}
                        setSelectedData={setRanah}
                        placeholder="ranah"
                    />

                    <MultipleSelectCheckBox
                        options={dataIndicator}
                        setSelectedData={setIndicator}
